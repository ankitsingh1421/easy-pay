...
//     return <div className="flex w-full pt-0 sm:flex-col sm:w-fit sm:pt-4 ">
...
//         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="duration-300 ease-in w-max size-10 hover:scale-125 ">
...
export default function Navcomp() {
    ...
    return (
        <div className="flex w-full pt-0 sm:flex-col sm:w-fit sm:pt-4 ">
            <NavItem css={getcss(0)} id={0} setIndex={setIndex} index={index} />
            <NavItem css={getcss(3)} id={3} setIndex={setIndex} index={index} url="home" text={"Home"} d={"m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25"} />
            <NavItem css={getcss(1)} id={1} setIndex={setIndex} index={index} url="Transaction" text={"Transaction"} d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
            <NavItem css={getcss(2)} id={2} setIndex={setIndex} index={index} url="wallet" text={"Wallet"} d={"M21 12a2.25 2.25 0 0 0-2.25-2.25H15a3 3 0 1 1-6 0H5.25A2.25 2.25 0 0 0 3 12m18 0v6a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 18v-6m18 0V9M3 12V9m18 0a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 9m18 0V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v3"} />
            ...
            <NavItem
                ...
                text={"Logout"}
                isLogout={true}
                d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6A2.25 2.25 0 0 0 5.25 5.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3-3H9"
            />
...
function NavItem(props: any) {
    ...
    function handleClick() {
        if (props.isLogout) {
            localStorage.clear(); // clear all stored data (token, state)
            nav("/logout"); // redirect to logout page
            return;
        }
    ...
    return (
        <div
            className={
                props.css +
                " flex sm:pl-3 pt-1 pr-2 flex-col justify-center items-center sm:items-start sm:justify-normal sm:flex-row " +
                ((props.id === props.index) ? "text-blue-700" : " ")
            }
            onClick={handleClick}
        >
            <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className={`w-max size-10 hover:scale-125 ease-in duration-300 ${props.isLogout ? "text-red-500" : ""}`}
            >
                <path strokeLinecap="round" strokeLinejoin="round" d={props.d} />
            </svg>

            <div className={`visible w-max sm:pl-5 pt-1 sm:text-2xl ${props.isLogout ? "text-red-500" : ""}`}>
                {props.text}
            </div>
        </div>
    );
